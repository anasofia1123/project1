...
weather <- read.csv(file.path("project1", "austin_weather.csv"))
netflix <- read_csv(file.path("project1", "mynetflixviewinghistory.csv"))
glimpse(weather)
glimpse(netflix)
```
...
```{R}
#filter and arrange
merged_data %>% group_by(date) %>% filter(temp_avg >= 70) %>% summarise(n = n()) %>% arrange(desc(n))
merged_data %>% group_by(date) %>% filter(temp_avg <= 70) %>% summarise(n = n()) %>% arrange(desc(n))
...
kable(totalsummary, format = "html", table.attr = "style='width:70%;'")  %>% 
...
merged_data %>% group_by(genre) %>% summarize(n = n()) %>%  knitr::kable() %>%  kableExtra::kable_styling() 
merged_data %>% group_by(date,temp_avg) %>% summarize(mean_precipiatation = mean(precipitation, na.rm = TRUE), mean_temp = mean(temp_avg, na.rm = T), n = n()) %>% tail() %>% knitr::kable() %>%  kableExtra::kable_styling()
